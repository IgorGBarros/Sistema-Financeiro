...
# Python da Microsoft Store funciona, mas o 'ensurepip' da criacao do venv roda
# dentro do container do app e costuma levar de um a dois minutos sem imprimir
# nada. Parece travado e nao esta.
if ($py.Caminho -like "*WindowsApps*") {
    Write-Host ""
    Write-Host "Aviso: este Python veio da Microsoft Store." -ForegroundColor Yellow
    Write-Host "A criacao do virtualenv fica sem saida por 1-2 minutos. NAO"
    Write-Host "interrompa com Ctrl+C. Instalar o Python de python.org deixa"
    Write-Host "esse passo em segundos."
}

# Pasta sincronizada com a nuvem e a causa mais comum de lentidao extrema aqui:
# o venv cria milhares de arquivos pequenos e o cliente de sincronizacao tenta
# subir cada um enquanto o pip escreve.
$raizTexto = "$raiz"
if (($raizTexto -like "*OneDrive*") -or ($raizTexto -like "*Dropbox*") -or
    ((Test-Path (Join-Path $env:USERPROFILE "OneDrive")) -and ($raizTexto -like "*\Documents\*"))) {
    Write-Host ""
    Write-Host "Aviso: o projeto esta em $raizTexto" -ForegroundColor Yellow
    Write-Host "Se essa pasta sincroniza com OneDrive, a instalacao pode travar."
    Write-Host "Mova para algo como C:\dev\sistema-financeiro se demorar demais."
}
...
if (-not (Test-Path $pythonVenv)) {
    ...
    Write-Host "    criando virtualenv (pode levar ate 2 minutos, aguarde)..."
    $inicio = Get-Date
    & $py.Caminho @($py.Args) -m venv .venv

    # Alternativa quando o ensurepip trava: cria o venv vazio, que e rapido, e
    # instala o pip num passo separado. Assim da para ver qual dos dois esta
    # lento, em vez de encarar um comando mudo.
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $pythonVenv)) {
        Write-Host "    a criacao normal falhou; tentando sem o pip..." -ForegroundColor Yellow
        if (Test-Path ".venv") { Remove-Item -Recurse -Force ".venv" }
        & $py.Caminho @($py.Args) -m venv .venv --without-pip
        if ($LASTEXITCODE -eq 0 -and (Test-Path $pythonVenv)) {
            Write-Host "    virtualenv criado; instalando o pip..."
            & $pythonVenv -m ensurepip --upgrade --default-pip
        }
    }
...
Causas comuns:
  ...
  1. Python da Microsoft Store: o 'ensurepip' roda no container do app e as
     vezes trava. Instale o Python de python.org e rode de novo.
  2. Pasta sincronizada com OneDrive: mova para C:\dev\sistema-financeiro.
  3. Antivirus bloqueando a escrita: adicione a pasta as excecoes.
"@
    }
    Write-Host "    virtualenv criado em backend\.venv ($([int]((Get-Date)-$inicio).TotalSeconds)s)"
}
